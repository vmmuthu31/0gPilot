...
export const workflowWorker = new Worker("workflows", async (job) => {
  ...
  emitWorkflowEvent(projectId, "STARTING", "Workflow queued and starting execution");
  
  const result = await executeWorkflow(prompt, projectId);
  ...
  emitWorkflowEvent(projectId, "COMPLETED", "Workflow execution finished successfully");
  return result;
}, { connection });
